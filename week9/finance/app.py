...
def index():
    ...
    # Get current price and value of each stock
    for stock in stocks:
...
def buy():
    ...
    if request.method == "POST":
        ...
        flash(
            f"Bought {shares} shares of {quote['symbol']} for {usd(total_cost)}!"
        )
...
def sell():
    ...
    if request.method == "POST":
        ...
        if not shares or not shares.isdigit() or int(shares) <= 0:
            return apology(
                "must provide a positive integer for shares",
                400
            )
        ...
        for stock in stocks:
            ...
            if stock["symbol"] == symbol:
                ...
                flash(
                    ...
                    f"for {usd(total_sale)}!"
                )

                return redirect("/")

        # Stock wasn't found in portfolio
        return apology("symbol not found", 400)
